...
char getPoint(Point& pt)
{
	...
	while (input != ESC) {
		if (input == SPECIAL) {
			input = _getch();
			// moves cursor around by arrow keys
			switch (input) {
			case LEFTARROW:
		...
		// handles whitespace keys
			...
			cout << input;
			gotoxy(row, col);
			// updates pointer to location user entered location at
			pt = { row, col };
			// returns the character user entered
			return input;
		}
		input = _getch();
	}
...
void treeRecursive(char canvas[][MAXCOLS], DrawPoint start, int height, int startAngle, int branchAngle, bool animate)
{
	...
	// draws 1 for depth 0 of tree, 2 for depth 1, 4 for depth 2, 8 for depth 3, and so on
...
{
	// base case
	// ends when the box would be too small to draw
	if (height <= 1)
		return;
	// recursive 
	drawBox(canvas, center, height, animate);
	// draws next box inside previous box
