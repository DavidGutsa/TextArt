...
*/
void loadCanvas(char canvas[][MAXCOLS], char fileName[]);
...
int main()
{
	...
	while (input != 'q' && input != 'Q') {
		...
		case 'M':
			...
			cin >> moveRow;
			cout << "Enter the col units to move: ";
			cin >> moveCol;
		...
		case 'L':
			clearLine(MAXROWS + 1, 100);
			cout << "Enter filename to be loaded: ";
			cin >> fileName;
			clearLine(MAXROWS + 1, 100);
			copyCanvas(undoCanvas, canvas);
			initCanvas(canvas);
...
void editCanvas(char canvas[][MAXCOLS])
{
	...
	while (input != ESC) {
		...
		if (input == SPECIAL) {
			...
			input = _getch();
		else if (input != '\n' && input != '\t' && input != '\r') { // handles whitespace keys
...
void moveCanvas(char canvas[][MAXCOLS], int rowValue, int colValue)
{
	...
	for (int i = 0; i < MAXROWS; i++) {
		for (int j = 0; j < MAXCOLS; j++) {
			if (i + rowValue > 0 && i + rowValue < MAXROWS && j + colValue < MAXCOLS && j + colValue > 0) // adds a character to the new array to the new position, and ignores the character if it would be out of bounds
...
void saveCanvas(char canvas[][MAXCOLS], char fileName[])
{
	...
	sprintf_s(filePath, "SavedFiles/%s.txt", fileName);
